                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <NavLink
                            key={item.name}
                            to={item.to}
                            className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium cursor-pointer ${isActive ? 'bg-indigo-100 text-indigo-700' : 'text-slate-700 hover:bg-slate-100'}`}
                        >
